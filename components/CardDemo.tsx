...
const Sparkles = () => {
  ...
  const getRandomValue = (index: number, seed: number) => {
    ...
    return ((Math.sin(index * seed) + 1) / 2);
  };
  ...
  return (
    <div className="absolute inset-0">
      {[...Array(12)].map((_, i) => {
        const randomTop = getRandomValue(i, 12.9898);
        const randomLeft = getRandomValue(i, 78.233);
        const randomDuration = getRandomValue(i, 45.164) * 2 + 4;
        const randomOpacity = getRandomValue(i, 64.467);

        return (
          <motion.span
            key={`star-${i}`}
            animate={{
              top: `calc(${randomTop * 100}% + ${(randomLeft * 2 - 1)}px)`,
              left: `calc(${randomLeft * 100}% + ${(randomTop * 2 - 1)}px)`,
